    ...
    public class TestBsonReader : BsonTestBase
    {
        ...
        public void TestReadBigDocument(){
            ...
            expected.Append("str", "test")
                .Append("int", 45)
                .Append("long", (long)46)
                .Append("num", 4.5).Add("date", DateTime.Today)
                .Append("_id", new OidGenerator().Generate()).Add("code", new Code("return 1;"))
                .Append("subdoc", new Document("a", 1).Append("b",2))                
                .Append("array", new String[]{"a","b","c","d"})
                .Append("codewscope", new CodeWScope("return 2;", new Document("c", 1)))
                .Append("binary", new Binary(new byte[]{0,1,2,3}))
                .Append("regex", new MongoRegex("[A-Z]")).Add("minkey", MongoMinKey.Value)
                .Append("maxkey", MongoMaxKey.Value).Add("symbol", new MongoSymbol("symbol"))
            ;
